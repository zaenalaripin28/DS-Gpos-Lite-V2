...
`components/Lozenge/lozenge.html`

Global tokens: `styles/tokens.css` · Tailwind: `tailwind.config.js` · Shared CSS: `styles/globals.css`, `styles/enhancements.css`

## Purpose

Label status berbentuk rounded rectangle untuk menandai kategori atau status item di tabel, kartu, atau daftar. Enam appearance dengan gaya subtle dan bold berbasis semantic tokens.

## Anatomy

* Root container (`.ds-lozenge`)
...
Untuk status semantik pada baris tabel, issue tracker, workflow, atau metadata item — bukan untuk angka hitung (gunakan Badge).
...
* Primary root class: .ds-lozenge. Key related classes: .ds-lozenge, .ds-lozenge--bold, .ds-lozenge--default, .ds-lozenge--inprogress, .ds-lozenge--moved, .ds-lozenge--new.
* Never create new variants, colors, spacing scales, or typography sizes unless explicitly requested.
* Match HTML structure and state handling from components/Lozenge/lozenge.html.
* Preserve semantic HTML, native form controls, and ARIA attributes from the implementation.

## Example Prompt For AI

```
Implement Lozenge for GPOS Lite Design System V2. Source: components/Lozenge/lozenge.html Root class: .ds-lozenge Appearances/variants: default, inprogress, moved, new, removed, success Stack: HTML + Tailwind CSS + tokens.css. No React. Copy exact class names from source.
```
