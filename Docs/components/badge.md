...
Label kecil untuk menampilkan informasi seperti jumlah, status, atau perubahan nilai. Tersedia dalam 6 varian appearance yang masing-masing memiliki makna semantik tersendiri.
...
Use Badge when the interaction pattern matches the documented implementation in components/Badge/badge.html. Compose with existing DS components; do not recreate styles.
...
* Primary root class: .ds-badge. Key related classes: .ds-badge, .ds-badge--default, .ds-badge--primary, .ds-badge--subtle, .ds-badge--important, .ds-badge--added.
* Never create new variants, colors, spacing scales, or typography sizes unless explicitly requested.
* Match HTML structure and state handling from components/Badge/badge.html.
* Preserve semantic HTML, native form controls, and ARIA attributes from the implementation.

## Example Prompt For AI

```
Implement Badge for GPOS Lite Design System V2. Source: components/Badge/badge.html Root class: .ds-badge Appearances/variants: default, primary, subtle, important, added, removed Stack: HTML + Tailwind CSS + tokens.css. No React. Copy exact class names from source.
```
