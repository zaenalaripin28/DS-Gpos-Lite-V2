...
`components/Avatar/avatar.html`

Global tokens: `styles/tokens.css` · Tailwind: `tailwind.config.js` · Shared CSS: `styles/globals.css`, `styles/enhancements.css`

## Purpose

Merepresentasikan identitas pengguna secara visual. Tersedia dalam 4 ukuran dengan presence indicator untuk menampilkan status online.
...
Use Avatar when the interaction pattern matches the documented implementation in components/Avatar/avatar.html. Compose with existing DS components; do not recreate styles.
...
* Primary root class: .ds-avatar. Key related classes: .ds-avatar, .ds-avatar__image, .ds-avatar__presence, .ds-avatar--xl, .ds-avatar--lg, .ds-avatar--md.
* Never create new variants, colors, spacing scales, or typography sizes unless explicitly requested.
* Match HTML structure and state handling from components/Avatar/avatar.html.
* Preserve semantic HTML, native form controls, and ARIA attributes from the implementation.

## Example Prompt For AI

```
Implement Avatar for GPOS Lite Design System V2. Source: components/Avatar/avatar.html Root class: .ds-avatar Appearances/variants: xl, lg, md, sm Sizes: xl, lg, md, sm Stack: HTML + Tailwind CSS + tokens.css. No React. Copy exact class names from source.
```
