...
var require_sync = __commonJS({
  "../../../node_modules/totalist/sync/index.js"(exports) {
    var { join: join2, resolve: resolve3 } = __require("path"), { readdirSync, statSync } = __require("fs");
...
var require_build2 = __commonJS({
  "../../../node_modules/sirv/build.js"(exports, module) {
    var fs = __require("fs"), { join: join2, normalize: normalize2, resolve: resolve3 } = __require("path"), { totalist } = require_sync(), { parse } = require_build(), { lookup } = require_mrmime(), noop = () => {
    };
    ...
    function viaCache(cache2, uri, extns) {
      ...
      for (; i < arr.length; i++)
        if (data = cache2[arr[i]]) return data;
    }
...
async function requiresProjectAnnotations(testConfig, finalOptions) {
  ...
  }).filter(Boolean).find((setupFile) => dirname(setupFile) === finalOptions.configDir ? readFileSync(setupFile, "utf-8").includes("setProjectAnnotations") : !1) ? (logBoxOnce(import_ts_dedent.dedent`
...
// src/vitest-plugin/agent-telemetry-reporter.ts
...
// src/vitest-plugin/agent-story-history-cache.ts
import { createFileSystemCache, resolvePathInStorybookCache } from "storybook/internal/common";
var CACHE_KEY = "agent-self-healing-story-history", historyCache = createFileSystemCache({
  basePath: resolvePathInStorybookCache("agent-self-healing"),
  ns: "storybook"
});
async function readStoryHistory() {
  return await historyCache.get(CACHE_KEY) ?? {};
}
async function mergeAndWriteStoryHistory(results) {
  let history = await readStoryHistory(), now = Date.now();
  for (let result of results) {
    let existing = history[result.storyId];
    (!existing || existing.timestamp <= now) && (history[result.storyId] = { ...result, timestamp: now });
  }
  return await historyCache.set(CACHE_KEY, history), Object.values(history).map(({ timestamp, ...rest }) => rest);
}

// ../../core/src/shared/utils/ai-checklist-flags.ts
...
async function readProjectScopedFlag(key, configDir) {
  try {
    ...
    if (isProjectScopedFlag(value) && value.configDir === resolve2(configDir))
      return value;
  } catch {
  }
}
...
// src/vitest-plugin/agent-telemetry-reporter.ts
var AgentTelemetryReporter = class {
  constructor(options) {
    this.testResults = [];
    this.startTime = Date.now();
    this.configDir = options.configDir, this.agent = options.agent;
  }
  onInit(ctx) {
    this.ctx = ctx;
  }
  ...
  onTestCaseResult(testCase) {
    let { storyId, reports } = testCase.meta();
    if (!storyId || isExampleStoryId(storyId))
      return;
    let testResult = testCase.result(), result = toStoryTestResult({
      storyId,
      statusRaw: testResult.state,
      reports,
      errors: testResult.errors
    });
    result && this.testResults.push(result);
  }
  async onTestRunEnd(testModules, unhandledErrors) {
    let cumulativeResults = await mergeAndWriteStoryHistory(this.testResults), analysis = analyzeTestResults(this.testResults, cumulativeResults), duration = Date.now() - this.startTime, testModulesErrors = testModules.flatMap((t) => t.errors()), unhandledErrorCount = unhandledErrors.length + testModulesErrors.length, runId = await getAiSetupRunId(this.configDir);
    telemetry(
      ...
      { configDir: this.configDir, stripMetadata: !0 }
    ), this.testResults = [];
  }
};
...
var WORKING_DIR = process.cwd(), defaultOptions = {
  ...
  return {
    ...
    storiesFiles: StoryIndexGenerator.storyFileNames(
      new Map(matchingStoryFiles.map(([specifier, cache2]) => [specifier, cache2]))
    )
  };
}, mdxStubPlugin = {
  ...
  let agent, withinAgenticSetupSession = !1, storybookTestPlugin = {
    ...
    async config(nonMutableInputConfig, { mode }) {
      isTelemetryModuleEnabled() && (agent = detectAgent(), withinAgenticSetupSession = !!agent && await isWithinInitialSession("ai-setup")), mode && (process.env.BUILD_TARGET = mode);
      try {
      ...
      let projectId = oneWayHash(finalOptions.configDir), internalSetupFiles = [
        ...
        cacheDir: resolvePathInStorybookCache2("sb-vitest", projectId),
        test: {
          ...
          provide: {
            ...
            [STORYBOOK_CORE_RENDER_ANALYSIS_PROVIDE_KEY]: !!process.env.STORYBOOK_COMPONENT_PATHS || withinAgenticSetupSession
          },
      ...
      }, config = mergeConfig(baseConfig, viteConfigFromStorybook);
      return (nonMutableInputConfig.test?.include?.length ?? 0) > 0 && (nonMutableInputConfig.test.include = [], console.log(
        import_picocolors.default.yellow(import_ts_dedent2.dedent`
    ...
    async configureVitest(context) {
      context.vitest.config.coverage.exclude.push("storybook-static"), telemetry2(
        "test-run",
        {
          runner: "vitest",
        ...
        { configDir: finalOptions.configDir }
      ), isTelemetryModuleEnabled() && agent && withinAgenticSetupSession && !process.env.STORYBOOK_INTERNAL_TEST_RUN && context.vitest.config.reporters.push(
