import CJS_COMPAT_NODE_URL_emjvob9t86o from 'node:url';
import CJS_COMPAT_NODE_PATH_emjvob9t86o from 'node:path';
import CJS_COMPAT_NODE_MODULE_emjvob9t86o from "node:module";

var __filename = CJS_COMPAT_NODE_URL_emjvob9t86o.fileURLToPath(import.meta.url);
var __dirname = CJS_COMPAT_NODE_PATH_emjvob9t86o.dirname(__filename);
var require = CJS_COMPAT_NODE_MODULE_emjvob9t86o.createRequire(import.meta.url);
...
import {
  esm_default,
  stringify
} from "./chunk-D4SIGUWV.js";
import {
  visit
} from "./chunk-T7DQEDES.js";
import {
  __commonJS,
  __esm,
  __export,
        ...
        if (value && typeof value == "object")
          if (Array.isArray(value)) {
            ...
            for (let i = 0; i < nodes.length; i += 1) {
              let item = nodes[i];
              isNode(item) && (this.visit(item, node2, key2, i) || i--);
            }
...
var ForInStatement, FunctionDeclaration, RestElement, BinaryExpression, ArrayExpression, BlockStatement, GENERATOR = {
  ...
  */
  ...
  ImportDeclaration(node2, state) {
    state.write("import ");
    let { specifiers, attributes } = node2, { length } = specifiers, i = 0;
    if (length > 0) {
      for (; i < length; ) {
        i > 0 && state.write(", ");
    ...
    if (this.Literal(node2.source, state), attributes && attributes.length > 0) {
      ...
      for (let i2 = 0; i2 < attributes.length; i2++)
        this.ImportAttribute(attributes[i2], state), i2 < attributes.length - 1 && state.write(", ");
      state.write(" }");
    }
    state.write(";");
  },
    ...
    if (state.write("export "), node2.declaration)
      ...
      if (state.write("}"), node2.source && (state.write(" from "), this.Literal(node2.source, state)), node2.attributes && node2.attributes.length > 0) {
        ...
        for (let i = 0; i < node2.attributes.length; i++)
          this.ImportAttribute(node2.attributes[i], state), i < node2.attributes.length - 1 && state.write(", ");
        state.write(" }");
      }
      state.write(";");
    }
  ...
  ArrayExpression: ArrayExpression = function(node2, state) {
    if (state.write("["), node2.elements.length > 0) {
      ...
      for (let i = 0; ; ) {
        ...
        if (element2 != null && this[element2.type](element2, state), ++i < length)
          state.write(", ");
        else {
...
function attachComments(tree, comments) {
  ...
  list3.length > 0 && walk2(tree, { comments: list3, index: 0 });
}
